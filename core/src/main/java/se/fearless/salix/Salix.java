...
public class Salix<T> {
    ...
    public void visitNodes(NodeVisitor nodeVisitor) {
        root.executeInformationVisitor(nodeVisitor);
    }

    public void visitEntries(EntryVisitor<T> entryVisitor) {
        entryLookup.values().forEach(entryVisitor::visit);
    }

    public interface EntryVisitor<T> {
        void visit(EntryView<T> entryView);
    }
}
